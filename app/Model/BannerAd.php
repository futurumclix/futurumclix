 ...
 */
class BannerAd extends AppModel {
/**
 * Display field
 *
 * @var string
 ...
 */
	public $hasMany = array(
		...
		'ImpressionHistory' => array(
			...
			'dependent' => true,
			'conditions' => array(
				'ImpressionHistory.model' => 'BannerAd',
			),
 ...
 */
	public function assignPack($packData, $ad_id = null, $reset_stats = true) {
		...
		$this->set('package_type', $packData['BannerAdsPackage']['type']);
		$this->set('expiry', $packData['BannerAdsPackage']['amount']);
 ...
 */
	public function addPack($packData, $ad_id = null) {
		...
		$ad = $this->findById($ad_id, array(
			'DATE_ADD(BannerAd.start, INTERVAL BannerAd.expiry DAY) as end',
		...
		if($ad['BannerAd']['package_type'] == 'Days' && $ad[0]['end'] < date('Y-m-d H:i:s')) {
			return $this->assignPack($packData, $ad_id, false);
		}

		$this->recursive = -1;
		$res = $this->updateAll(array(
			'BannerAd.expiry' => '`BannerAd`.`expiry` + '.$packData['BannerAdsPackage']['amount'],
		), array(
			'BannerAd.id' => $ad_id,
			'BannerAd.package_type' => $packData['BannerAdsPackage']['type'],
		));
 ...
 */
	public function buy($ad_id, $package_id) {
		...
		$packModel = ClassRegistry::init('BannerAdsPackage');
		...
		$auto_approve = ClassRegistry::init('Settings')->fetchOne('bannerAdsAutoApprove', 0);
