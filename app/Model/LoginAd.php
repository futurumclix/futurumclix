 ...
 */
class LoginAd extends AppModel {
/**
 * Display field
 *
 * @var string
	...
	public $validate = array(
		...
		'image_url' => array(
			'maxLength' => array(
				...
				'message' => 'Image URL cannot be longer than 512 characters',
				'allowEmpty' => false,
			),
			'url' => array(
				'rule' => array('url', true),
 ...
 * @return boolean
 */
	public function assignPack($packData, $ad_id = null, $reset_clicks = true) {
		$this->clear();
		...
		$this->set('package_type', $packData['LoginAdsPackage']['type']);
		$this->set('expiry', $packData['LoginAdsPackage']['amount']);
 ...
 */
	public function addPack($packData, $ad_id = null) {
		...
		$ad = $this->findById($ad_id, array(
			'DATE_ADD(LoginAd.start, INTERVAL LoginAd.expiry DAY) as end',
		...
		if($ad['LoginAd']['package_type'] == 'Days' && $ad[0]['end'] < date('Y-m-d H:i:s')) {
			return $this->assignPack($packData, $ad_id, false);
		}

		$this->recursive = -1;
		$res = $this->updateAll(array(
			'LoginAd.expiry' => '`LoginAd`.`expiry` + '.$packData['LoginAdsPackage']['amount'],
		), array(
			'LoginAd.id' => $ad_id,
			'LoginAd.package_type' => $packData['LoginAdsPackage']['type'],
		));
 ...
 */
	public function buy($ad_id, $package_id) {
		...
		$packModel = ClassRegistry::init('LoginAdsPackage');
		...
		$auto_approve = ClassRegistry::init('Settings')->fetchOne('loginAdsAutoApprove', 0);
 ...
 */
	public function getStatistics($adId) {
		$this->contain(array('ClickHistory'));
