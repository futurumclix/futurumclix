...
class RequestObject extends Aro {
	...
	public $validate = array(
		'alias' => array(
			'rule' => 'isUnique',
			'allowEmpty' => false,
			'message' => 'Alias should be unique'
		),
		'maxLength' => array(
			'rule' => array('maxLength', 200),
			'message' => 'Alias should be shorter than 200 characters.',
			),
		);
	...
	public function addObject($alias, $parent_id = null) {
		$query = array('alias' => $alias, 'parent_id' => $parent_id);

		$result = $this->find('first', array('conditions' => $query));
	...
	public function addChildObject($alias, $parent_id, $model, $foreignKey) {
		$query = array('alias' => $alias, 'parent_id' => $parent_id, 'model' => $model, 'foreign_key' => $foreignKey);
		...
		return $this->cache(array(__METHOD__, $user_id), function ($self) use ($user_id, $filter) {
			...
			if($permissions = $self->getPermissions($user_id)) {
				foreach($permissions as $permission) {
					...
					$crud[$permission['ControlObject']['alias']] = array('create' => ($permission['ObjectPermission']['_create'] >= 0), 'read' => ($permission['ObjectPermission']['_read'] >= 0), 'update' => ($permission['ObjectPermission']['_update'] >= 0), 'delete' => ($permission['ObjectPermission']['_delete'] >= 0));
				}
			}
	...
	public function getRoles($user_id) {
		$results = $this->find('all', array('conditions' => array('RequestObject.foreign_key' => $user_id, 'RequestObject.model' => 'User',)));

		if($results) {
			// Grab the direct parents (or roles)
			$results = $this->find('all', array('conditions' => array('RequestObject.id' => Hash::extract($results, '{n}.RequestObject.parent_id'))));

			// Also grab the children of the parents since the user has access to them as well
			$results = array_merge($results, $this->find('all', array('conditions' => array('RequestObject.parent_id' => Hash::extract($results, '{n}.RequestObject.id'), 'RequestObject.model' => null, 'RequestObject.foreign_key' => null))));
		}
	...
	public function getStaff() {
		return $this->find('all', array('conditions' => array('RequestObject.parent_id' => array_keys($this->getList()), 'RequestObject.foreign_key !=' => null, 'RequestObject.model' => USER_MODEL), 'contain' => array('User', 'Parent')));
	}
	...
	public function getStaffByAlias($alias) {
		...
		return $this->find('all', array('conditions' => array('RequestObject.parent_id' => $access['RequestObject']['id'], 'RequestObject.foreign_key !=' => null, 'RequestObject.model' => USER_MODEL), 'contain' => array('User', 'Parent'), 'cache' => array(__METHOD__, $alias)));
	}
	...
	public function hasAlias($alias) {
		return (bool)$this->find('count', array('conditions' => array('RequestObject.alias' => $alias), 'cache' => array(__METHOD__, $alias), 'cacheExpires' => '+24 hours'));
	}
	...
	public function isChildOf($user_id, $fk) {
		...
		try {
			$aros = $this->node(array('model' => USER_MODEL, 'foreign_key' => $user_id));
		} catch(Exception $e) {
			return false;
		}
	...
	/**
	 * Check if user tries to remove predefined group
	 *
	 * @param bool $cascade
	 * @return bool
	 */
	public function beforeDelete($cascade = true) {
		if(!isset($this->data[$this->alias]['alias'])) {
			$this->read(array('alias'));
		}

		if(in_array($this->data[$this->alias]['alias'], Configure::read('Admin.aliases'))) {
			return false;
		}
		return true;
	}
