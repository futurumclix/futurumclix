...
class Post extends ForumAppModel {
    ...
    public function getViewURL($id) {
      $this->id = $id;
      $this->contain(array('Topic' => 'slug'));
      $this->read(array('id', 'topic_id'));
      return array('admin' => false, 'plugin' => 'forum', 'controller' => 'topics', 'action' => 'view', $this->data['Topic']['slug'], '#' => 'post-'.$id);
    }
    ...
    public function addPost($data) {
        ...
        if ($this->validates()) {
            ...
            if (($secondsLeft = $this->checkFlooding($settings['postFloodInterval'])) > 0 && !$isSuper) {
                return $this->invalid('content', 'You must wait %s more second(s) till you can post a reply', $secondsLeft);

            } else if ($this->checkHourly($settings['postsPerHour']) && !$isSuper) {
                ...
                $this->User->contain(array('ActiveMembership' => array('Membership' => array('points_enabled', 'points_per_post'))));
                $user = $this->User->findById($data['user_id'], array('id'));
                if($user['ActiveMembership']['Membership']['points_enabled']) {
                  $this->User->pointsAdd($user['ActiveMembership']['Membership']['points_per_post'], $user['User']['id']);
                }
