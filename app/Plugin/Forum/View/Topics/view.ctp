                ...
                <tbody>
                    ...
                    <tr class="postline <?php if ($isBuried) echo 'is-buried'; ?>" id="post-<?php echo $post_id; ?>">
                        <td class="post-time">
                            <?php echo $this->Html->tag('time',
                                ...
                                array('data-toggle' => 'tooltip', 'data-original-title' => $post['Post']['created'], 'datetime' => $post['Post']['created'], 'data-placement' => 'top')
                            ); ?>
                        </td>
                        <td>
                                ...
                                if ($user) {
                                    ...
                                    if ($topic['Topic']['firstPost_id'] == $post_id) {
                                        if ($isMod || ($topic['Topic']['status'] && $user['id'] == $post['Post']['user_id'])) {
                                            $links[] = $this->Html->link('<span class="fa fa-wrench"></span>', array('controller' => 'topics', 'action' => 'edit', $topic['Topic']['slug'], (!empty($topic['Poll']['id']) ? 'poll' : '')), array('escape' => false, 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Edit Topic')));
                                        }

                                        if ($isMod) {
                                            $links[] = $this->Html->link('<span class="fa fa-times"></span>', array('controller' => 'topics', 'action' => 'delete', $topic['Topic']['slug']), array('escape' => false, 'confirm' => __d('forum', 'Are you sure you want to delete?'), 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Delete Topic')));
                                        }

                                        $links[] = $this->Html->link('<span class="fa fa-flag"></span>', array('controller' => 'topics', 'action' => 'report', $topic['Topic']['slug']), array('escape' => false, 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Report Topic')));
                                    } else {
                                        if ($isMod || ($topic['Topic']['status'] && $user['id'] == $post['Post']['user_id'])) {
                                            $links[] = $this->Html->link('<span class="fa fa-wrench"></span>', array('controller' => 'posts', 'action' => 'edit', $post_id), array('escape' => false, 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Edit Post')));
                                            $links[] = $this->Html->link('<span class="fa fa-times"></span>', array('controller' => 'posts', 'action' => 'delete', $post_id), array('escape' => false, 'confirm' => __d('forum', 'Are you sure you want to delete?'), 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Delete Post')));
                                        }

                                        $links[] = $this->Html->link('<span class="fa fa-flag"></span>', array('controller' => 'posts', 'action' => 'report', $post_id), array('escape' => false, 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Report Post')));
                                    }

                                    if ($canReply) {
                                        $links[] = $this->Html->link('<span class="fa fa-quote-left"></span>', array('controller' => 'posts', 'action' => 'add', $topic['Topic']['slug'], $post_id), array('escape' => false, 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Quote')));
                                    }
                                }

                                $links[] = $this->Html->link('<span class="fa fa-link"></span>', '#post-' . $post_id, array('escape' => false, 'class' => 'js-tooltip', 'data-original-title' => __d('forum', 'Link To This')));
                            ...
                            <?php if ($user) {
                                if ($settings['enablePostRating'] && (!$hasRated || $settings['showRatingScore'])) { ?>
                                    <div id="post-ratings-<?php echo $post_id; ?>" class="post-ratings<?php if ($hasRated) echo ' has-rated'; ?>">
                                        <?php if (!$hasRated) { ?>
                                            <a href="javascript:;" onclick="return Forum.ratePost(<?php echo $post_id; ?>, 'up');" class="rate-up js-tooltip" data-original-title="<?php echo __d('forum', 'Rate Up'); ?>">
                                                <span class="fa fa-arrow-up"></span>
                                            </a>
                                        <?php }
										$score = number_format($post['Post']['score']);
                                        if ($settings['showRatingScore'] && $score >= 0) { ?>
                                            <span class="rating"><i class="fa fa-thumbs-up"></i> <?php echo number_format($post['Post']['score']); ?></span>
                                        <?php } else { ?>
											<span class="rating"><i class="fa fa-thumbs-down"></i> <?php echo number_format($post['Post']['score']); ?></span>
										<?php }

                                        if (!$hasRated) { ?>
                                            <a href="javascript:;" onclick="return Forum.ratePost(<?php echo $post_id; ?>, 'down');" class="rate-down js-tooltip" data-original-title="<?php echo __d('forum', 'Rate Down'); ?>">
                    ...
                    <tr>
                        <td class="span-2">
							...
							<h6 class="btn btn-primary btn-sm"><?php echo $post['User']['ActiveMembership']['Membership']['name']; ?></h6>
                            <?php if (!$isBuried) {
                                echo $this->Forum->avatar($post) ?>
                            <?php if (!empty($post['User']['ForumModerator'])) { ?>
                            <h4 class="username">Moderator</h4>
                            <?php } ?>
    ...
    if ($settings['enableQuickReply'] && $canReply) { ?>
        ...
        <div id="quick-reply" class="panel quick-reply">
            ...
            <div class="panel-body padding10">
				...
				echo '<div class="text-xs-right" style="padding-top: 10px;">';
                echo $this->Form->submit(__d('forum', 'Post Reply'), array('class' => 'btn btn-primary'));
				echo '</div>';
                echo $this->Form->end(); ?>
            </div>
        </div>
