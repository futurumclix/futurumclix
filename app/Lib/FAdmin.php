...
class FAdmin {
	...
	public static function getPlugin($plugin) {
		return self::cache(array(__METHOD__, $plugin), function () use ($plugin) {
			...
			return array('title' => $plugin, 'path' => $path, 'slug' => Inflector::underscore($plugin), 'models' => FAdmin::getPluginModels($plugin));
		});
	}
	...
	public static function getPluginModels($plugin) {
		return self::cache(array(__METHOD__, $plugin), function () use ($plugin) {
			...
			foreach($models as $model) {
				list($plugin, $model, $id, $class) = FAdmin::parseName($plugin . '.' . $model);

				if(in_array($id, $ignore)) {
					continue;
				}

				$object = FAdmin::introspectModel($id);

				if(!$object) {
					continue;
				}

				$map[] = array_merge($object->admin, array('id' => $id, 'title' => $object->pluralName, 'alias' => $model, 'class' => $class, 'url' => Inflector::underscore($id), 'installed' => FAdmin::isModelInstalled($id), 'group' => $object->useDbConfig));
			}
	...
	public static function parseName($model) {
		return self::cache(array(__METHOD__, $model), function () use ($model) {
			...
			return array($plugin, // plugin name, includes Core
	...
	public static function introspectModel($model) {
		return self::cache(array(__METHOD__, $model), function () use ($model) {
			list($plugin, $model, $id, $class) = FAdmin::parseName($model);
			$core = Configure::read('Admin.coreName');
			...
			if(is_array($settings)) {
				...
				if(isset($settings['fileFields']) && isset($settings['imageFields'])) {
					$settings['fileFields'] = array_merge($settings['fileFields'], $settings['imageFields']);
				} elseif(isset($settings['imageFields'])) {
					$settings['fileFields'] = $settings['imageFields'];
				} elseif(!isset($settings['fileFields'])) {
					$settings['fileFields'] = array();
				}

				if(isset($settings['hideFormFields'])) {
					$settings['hideFormFields'] = array_merge($settings['hideFormFields'], $hideFormFields);
				} else {
					$settings['hideFormFields'] = $hideFormFields;
				}

				if(isset($settings['hideTableFields'])) {
					$settings['hideTableFields'] = array_merge($settings['hideTableFields'], $hideTableFields);
				} else {
					$settings['hideTableFields'] = $hideTableFields;
				}

				if(isset($settings['hideReadFields'])) {
					$settings['hideReadFields'] = array_merge($settings['hideReadFields'], $hideReadFields);
				} else {
					$settings['hideReadFields'] = $hideReadFields;
				}
