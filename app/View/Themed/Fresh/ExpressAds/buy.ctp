	...
	<div class="uk-grid">
		...
		<div class="uk-width-3-4@m">
			...
			<div class="uk-margin uk-text-center">
				<?=$this->UserForm->getGatewaysButtons($activeGateways, array('prices' => $packagesData[key($packagesData)]['ExpressAdsPackage']['prices']))?>
			</div>
		</div>
		<?=$this->UserForm->end()?>
		<?php endif; ?>
...
<?php
	...
	switch(Data[pack_id]['ExpressAdsPackage']['type']) {
	case 'Clicks':
	...
	$('#price').attr('placeholder', formatCurrency(Data[pack_id]['ExpressAdsPackage']['price']));
	$('#pricePer').attr('placeholder', formatCurrency(Data[pack_id]['ExpressAdsPackage']['price_per']));
	
	$.each(Data[pack_id]['ExpressAdsPackage']['prices'], function(key, value) {
	if(value == 'disabled') {
