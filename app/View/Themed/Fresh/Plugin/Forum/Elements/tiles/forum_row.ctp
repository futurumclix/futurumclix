...
<tr id="forum-<?php echo $forum['id']; ?>">
	...
	<td class="col-activity">
		<?php if (!empty($forum['LastTopic']['id'])) {
			...
			echo $this->Html->link('<span class="fa fa-external-link"></span>', array('controller' => 'topics', 'action' => 'view', $forum['LastTopic']['slug'], 'page' => ($forum['LastTopic']['post_count'] > $settings['postsPerPage']) ? ceil($forum['LastTopic']['post_count'] / $settings['postsPerPage']) : 1, '#' => 'post-' . $forum['lastPost_id']), array('escape' => false)); ?><br>
