<div class="col-md-12">
	<div class="title">
		<h2><?=__d('admin', 'Add new express ad:')?></h2>
	</div>
	<?=$this->AdminForm->create('ExpressAd', array(
	...
	<?php else: ?>
		<div class="form-group">
			...
			<div class="col-sm-2">
				<input type="button" class="btn btn-default" style="vertical-align: top;" value="<?=__d('admin', 'Select All')?>" onclick="selectAll('ExpressAdTargettedLocations')"/>
			</div>
		</div>
	<?php endif; ?>
	<div class="text-center">
		<button class="btn btn-primary"><?=__d('admin', 'Save')?></button></td>
	</div>
</div>
<?=$this->AdminForm->end()?>
<?php $this->Js->buffer("
	function change_expiry() {
		if($('#ExpressAdPackageType').val() == 'Days') {
			$('#expiry').hide();
			$('#expiryDate').show();
		} else {
			$('#expiry').show();
	...
	change_expiry();
	$('#ExpressAdPackageType').on('change', change_expiry);
");
