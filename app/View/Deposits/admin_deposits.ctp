<div class="col-md-12">
	<div class="title">
		<h2><?=__d('admin', 'Deposit list')?></h2>
	</div>
	<?=
		$this->AdminForm->create(false, array(
			'url' => array('action' => 'deposits'),
		...
		<table class="table table-striped table-hover">
			<tbody>
				<tr>
					...
					<th><?=$this->Paginator->sort('amount', __d('admin', 'Payment amount')); ?></th>
					<th><?=$this->Paginator->sort('deposit_amount', __d('admin', 'Deposit amount')); ?></th>
				...
				<tr>
					...
					<td><?=h($this->Currency->format($deposit['Deposit']['deposit_amount'])); ?>&nbsp;</td>
					...
					<td class="actions">
						<?=
							$this->AdminForm->postLink(
								'<i class="fa fa-minus-square fa-lg" data-toggle="tooltip" data-placement="top" title="'.__d('admin', 'Delete deposit').'"></i>',
						...
						<?php if($deposit['User']['id'] != null): ?>
							<?=
								$this->AdminForm->postLink(
									'<i class="fa fa-trash fa-lg" data-toggle="tooltip" data-placement="top" title="'.__d('admin', 'Delete deposit and remove cash from purchase balance').'"></i>',
									array('action' => 'cancel', $deposit['Deposit']['id']), 
									array('escape' => false), 
									__d('admin', 'Are you sure you want to delete # %s?', $deposit['Deposit']['id'])
								);
							?>
							<?php if($deposit['Deposit']['status'] == 'Pending'): ?>
								<?=
									$this->AdminForm->postLink(
										'<i class="fa fa-plus fa-lg" data-toggle="tooltip" data-placement="top" title="'.__d('admin', 'Accept deposit').'"></i>',
							...
							<?php endif; ?>
						<?php endif; ?>
						<?php if($deposit['Deposit']['refunds'] && $deposit['Deposit']['status'] != 'Refunded'): ?>
							<?php if($deposit['User']['id'] != null): ?>
								<?=
									$this->AdminForm->postLink(
										'<i class="fa fa-refresh fa-lg" data-toggle="tooltip" data-placement="top" title="'.__d('admin', 'Refund deposit and remove cash from purchase balance').'"></i>',
										array('action' => 'refund', $deposit['Deposit']['id']), 
										array('escape' => false), 
										__d('admin', 'Are you sure you want to refund and cancel # %s?', $deposit['Deposit']['id'])
									);
								?>
							<?php else: ?>
								<?=
		...
		<div class="input-group">
			...
			<?=
				$this->AdminForm->input('Action', array(
					...
					'id'=> 'selectMassAction',
					'options' => array(
						'delete' => __d('admin', 'Delete deposits'),
