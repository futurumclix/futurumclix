						...
						<div class="padding30-col">
							...
							<div class="col-md-8 col-md-offset-2 margin30-top">
								...
								<div class="gatewaybuttons text-xs-center">
									<?=$this->UserForm->getGatewaysButtons($activeGateways, array('prices' => $packagesData[key($packagesData)]['LoginAdsPackage']['prices']))?>
								</div>
							</div>
							<?=$this->UserForm->end()?>
						</div>
...
<?php
	...
	$this->Js->buffer("
		...
		function calcPrice() {
			...
			switch(Data[pack_id]['LoginAdsPackage']['type']) {
				case 'Clicks':
			...
			$('#price').attr('placeholder', formatCurrency(Data[pack_id]['LoginAdsPackage']['price']));
			$('#pricePer').attr('placeholder', formatCurrency(Data[pack_id]['LoginAdsPackage']['price_per']));

			$.each(Data[pack_id]['LoginAdsPackage']['prices'], function(key, value) {
				if(value == 'disabled') {
