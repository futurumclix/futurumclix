...
<?=$this->UserForm->create(false, array('id' => 'reportForm', 'url' => array('controller' => 'explorer_ads', 'action' => 'report', $adId)))?>
...
<div class="modal-footer">
	<div class="text-xs-right" style="padding: 10px;">
		<button id="reportButton" class="btn btn-danger"><?=__('Report Ad')?></button>
	</div>
</div>
<?=$this->UserForm->end()?>
<script>
	$('#reportForm').submit(function(event) {
		...
		$.ajax({
			type: "POST",
			data: dataStr,
			url: '<?=Router::url(array('controller' => 'explorer_ads', 'action' => 'report', $adId))?>',
